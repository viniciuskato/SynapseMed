  ...
  return (
    <div className="w-full max-w-[1680px] mx-auto space-y-7 pb-12">
      ...
      {activeTab === 'errors' ? (
        ...
        <>
      {/* ── 2. Card de Sugestão para Hoje: Factual e Baseado em Contagens Reais ── */}
      <div className="p-5 sm:p-6 rounded-3xl bg-slate-900 dark:bg-[#0B132B] border border-slate-700/80 dark:border-[#1E2D4A] elev-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-5 relative overflow-hidden">
        <div className="space-y-1.5 relative z-10 flex-1">
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-teal-500/15 text-teal-300 text-[11px] font-bold border border-teal-500/30">
          ...
          <h3 className="text-base sm:text-lg font-bold text-white tracking-tight">
            {dueCards.length > 0
              ? `Revisar ${dueCards.length} flashcard${dueCards.length > 1 ? 's' : ''} pendente${dueCards.length > 1 ? 's' : ''}`
              : errorLogs.length > 0
              ? `Revisar ${errorLogs.length} questão(ões) no Caderno de Erros`
              : 'Praticar novas questões do acervo'}
          </h3>
          <p className="text-xs text-slate-300 max-w-2xl leading-relaxed">
            {dueCards.length > 0
              ? 'Existem cartões com revisão agendada para hoje no algoritmo de repetição espaçada.'
              : errorLogs.length > 0
              ? 'Existem questões assinaladas para revisão após erro ou dúvida no seu caderno.'
              : 'Nenhum flashcard com revisão vencida ou pendência no caderno de erros no momento.'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 shrink-0 relative z-10 w-full sm:w-auto">
          {dueCards.length > 0 ? (
            <button
              type="button"
              onClick={onStartSRS}
              className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-teal-500 hover:bg-teal-400 text-slate-950 font-bold text-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer"
            >
              <Brain className="w-3.5 h-3.5" />
              <span>Abrir Revisão SRS ({dueCards.length})</span>
            </button>
          ) : errorLogs.length > 0 ? (
            <button
              type="button"
              onClick={() => handleSwitchTab('errors')}
              className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-rose-500 hover:bg-rose-400 text-white font-bold text-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer"
            >
            ...
            <button
              type="button"
              onClick={() => onSelectView('questions')}
              className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-100 border border-slate-600 font-bold text-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer"
            >
              <HelpCircle className="w-3.5 h-3.5 text-teal-400" />
              <span>Ir para Questões</span>
            </button>
          )}
        </div>
      </div>
