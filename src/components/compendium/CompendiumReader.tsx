...
export const CompendiumReader: React.FC<CompendiumReaderProps> = ({
  ...
  useEffect(() => {
    // Se o compêndio já foi 100% concluído, não deve manter atalho de leitura em andamento
    const totalSecs = compendium.sections?.length || 1;
    if (readSectionIds.length >= totalSecs) {
      StorageService.saveLastReadingSession(null);
      return;
    }
    ...
    // Ao retomar ou navegar para nova seção, remove qualquer dispensa anterior deste compêndio
    StorageService.clearDismissedReadingShortcut();
  }, [compendium, activeSectionId, discipline?.name, theme?.name, readSectionIds]);
  ...
  const handleToggleRead = async (sectionId: string) => {
    ...
    if (newPercent === 100) {
      ...
      // Ao atingir 100% de leitura, o atalho para continuar leitura incompleta é invalidado
      StorageService.saveLastReadingSession(null);
      StorageService.clearDismissedReadingShortcut();
    }
  };
