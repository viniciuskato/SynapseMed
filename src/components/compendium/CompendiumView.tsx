...
interface CompendiumViewProps {
  ...
  readingWarningMessage?: string | null;
  onClearReadingWarning?: () => void;
}
...
export const CompendiumView: React.FC<CompendiumViewProps> = ({
  ...
  onReturnToQuestions,
  lastReadingSession,
  readingWarningMessage,
  onClearReadingWarning,
}) => {
  ...
  return (
    <div className="space-y-6 w-full max-w-[1600px] mx-auto">
      {/* ── Aviso contextual de leitura indisponível ─────────────── */}
      {readingWarningMessage && (
        <div
          role="alert"
          className="p-3 sm:px-4 sm:py-3 rounded-2xl bg-amber-500/10 dark:bg-amber-950/40 border border-amber-500/30 dark:border-amber-700/40 elev-xs flex items-center justify-between gap-3 animate-in fade-in slide-in-from-top-2"
        >
          ...
          {onClearReadingWarning && (
            <button
              type="button"
              onClick={onClearReadingWarning}
              aria-label="Fechar aviso"
              className="p-1 rounded-lg text-amber-700 dark:text-amber-300 hover:bg-amber-500/20 transition-colors shrink-0 cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
          )}
